...
export default {
  ...
  */
  ...
  setupRunSettingsUI(element){
    ...
    //Add library loader support
    document.getElementById('derunsettings-txt-libraryloader').addEventListener("drop", function(event){
       event.preventDefault();
       var filePath = event.dataTransfer.getData("initialpath");
       if (filePath){
         that.currentConfPaths.libraryLoader = filePath;
         console.log("Library loader: " + filePath);
         document.getElementById('derunsettings-txt-libraryloader').value = that.formatPath(filePath);
       }
    });
    atom.tooltips.add(document.getElementById('derunsettings-txt-libraryloader'), {title: 'Drop the javascript file from Project.'});

    // add native storage directory
    document.getElementById('derunsettings-txt-localdatabase').addEventListener("drop", function(event){
       event.preventDefault();
       var filePath = event.dataTransfer.getData("initialpath");
       if (filePath){
         that.currentConfPaths.localdatabase = filePath;
         console.log("localdatabase folder: " + filePath);
         document.getElementById('derunsettings-txt-localdatabase').value = that.formatPath(filePath);
       }
    });
    atom.tooltips.add(document.getElementById('derunsettings-txt-localdatabase'), {title: 'Drop the folder from Project treeview.'});
  ...
  setupButtons(){
    ...
    document.getElementById('derunsettings-btn-run').addEventListener("click", function(event){
      ...
      var libraryLoader = that.currentConfPaths.libraryLoader;
      var localdatabase = that.parsePath(document.getElementById('derunsettings-txt-localdatabase').value);
      that.currentConf = Config.createNewConfig(assetsPath, mockImpl, indexFile, osType, deviceToggle);
      that.currentConf.libraryLoader = libraryLoader;
      that.currentConf.localdatabase = localdatabase;
  ...
  loadCurrentConf(element, configuration){
    this.populateRunSettingsUI(this.formatPath(this.currentConf.absolutePaths.rootFolder),
                               ...
                               this.formatPath(this.currentConf.absolutePaths.libraryLoader),
                               this.formatPath(this.currentConf.absolutePaths.localdatabase)
                             );
  },
  ...
  setupDefaultRunSettingsUI(element){
    ...
    this.populateRunSettingsUI(defaultRootPath, defaultIndex, defaultMockImpl, false, "ios",null,"${Prj}/localDatabase");
  },

  populateRunSettingsUI(rootFolder, indexFile, mockImpl, runOnPhysicalDev, emulatedOS,libraryLoader,localdatabase){
    ...
    document.getElementById('derunsettings-txt-libraryloader').value = libraryLoader;
    document.getElementById('derunsettings-txt-localdatabase').value = localdatabase;
  },
