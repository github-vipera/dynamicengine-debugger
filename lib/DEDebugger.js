...
export default class DEDebugger {
  ...
  startConnectionListening(settings){
    ...
    io.on('connection', function(socket){
      ...
      eventBus.publish("LOG", { 'message' : 'on debugger session connection'});
      that.socketList.push(socket);
      protocolHandler = new ProtocolHandler(socket,settings);
      ...
      socket.on('de_debug_hn_call', function(msg){
        ...
        eventBus.publish("LOG", { 'remoteCall' : msg });
    ...
    expressApp.get('/__dedebugger/**', function(req,res){
      ...
      if (deInfoHeader || that.settings.DebugOnDevice==true){
    ...
    http.listen(this.settings.DebugPort, function(){
      ...
      //openChromeWindow();
      eventBus.publish("DEBUGGER_STARTED", { 'status' : 'started'});
    });
  ...
  stopDebugger(){
    ...
    eventBus.publish("DEBUGGER_STOPPED", { 'status' : 'stopped'});
  }
